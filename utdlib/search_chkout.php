...
?>
	...
	var regbranch = new RegExp("^[0-9]{1,2}$");
	...
	var chkbranch=regbranch.test(document.getElementById("branchid").value);
	
	if(chkbk | chktitle | chkauthor | chkbranch)
	return true;
	else{
	...
	document.getElementById("branchid").style.border = "1px solid #F70A26";
	return false;
	}
	}
	...
	<form name="searchBook" method="post" onSubmit="return validateForm();">
	 <h2 align=left>Search a Book to Check-out by:</h2> 
	...
	placeholder="Please enter only characters (Min: 2 chars)" value="<?php echo (isset($_POST['author']) && isset($_SESSION['bookid_exists'])) ? $_POST['author'] : '' ?>"> <br> <b><i>OR <tr><tr><tr>

	<tr><td valign=top><font size=4px>Branch Id: <td><input type="text" name="branchid" maxlength=3 id="branchid" style="border: 1px solid #878b87"
	placeholder="Please enter only digits" value="<?php echo (isset($_POST['branchid']) && isset($_SESSION['bookid_exists'])) ? $_POST['branchid'] : '' ?>"> <tr><tr><tr>

	<tr><tr><tr><tr>
	<tr><td><td><input type="submit" id="submit" name="submit" value="Search Book"><br><br>
	</table>
	</form>
	...
	$fetchq = " SELECT books.book_id, books.title, books.author_name, books.branch_id, books.no_of_copies as No_Of_Copies, 
				...
				WHERE books.book_id	LIKE '%$bookid%' AND books.title LIKE '%$title%' AND books.author_name LIKE '%$author%' AND 
				books.branch_id LIKE '%$branchid%'";
    ...
    if($num_rows<1){
	echo "<br><br><br><br><font color='red' ><h2 align=center>No Such Book Found!</h2></font>";
	}
	else{
	//session_unset();
	
	echo "<br><br><br><form name='search_chkout' id='search_chkout' action='chkout.php' onSubmit='return loadCheckout();' method='post'>
			<br><br>
	...
	echo "<input type=hidden name='searchchkout' id='searchchkout' value='yes'>";
